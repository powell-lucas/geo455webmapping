...
/* Impact Layer Classify Change */
function getImpactRadius(deaths) {
    if (deaths >= 25) return 30;
    if (deaths >= 10) return 20;
    if (deaths >= 3)  return 12;
    return 6;
}
...
function buildImpactLayer(geojson) {
    ...
    for (var i = 0; i < geojson.features.length; i++) {
        ...
        /* Classed Radius Change */
        var radius = getImpactRadius(deaths);
...
/* Legend (change with each layer selection) */
...
function updateLegend() {
    ...
    var intensityOn  = map.hasLayer(intensityLayer);
    var frequencyOn  = map.hasLayer(frequencyLayerGroup);
    var impactOn     = map.hasLayer(impactLayer);

    /* Impact Legend */
    if (impactOn) {
        div.innerHTML =
            "<b>Tornado Impact</b><br><b>(Fatalities)</b><br>" +
            '<span class="legend-circle" style="width:12px;height:12px;"></span> 1 - 2 deaths<br>' +
            '<span class="legend-circle" style="width:24px;height:24px;"></span> 3 - 9 deaths<br>' +
            '<span class="legend-circle" style="width:40px;height:40px;"></span> 10 - 24 deaths<br>' +
            '<span class="legend-circle" style="width:60px;height:60px;"></span> 25+ deaths<br>';
        return;
    }
    ...
    if (frequencyOn) {
        div.innerHTML =
            "<b>Tornado Frequency</b><br>" +
            '<span style="background:#4575b4;opacity:0.8;" class="legend-box"></span> Low<br>' +
            '<span style="background:#ffffbf;opacity:0.8;" class="legend-box"></span> Moderate<br>' +
            '<span style="background:#d73027;opacity:0.8;" class="legend-box"></span> High<br>' +
            '<span style="background:#800026;opacity:0.8;" class="legend-box"></span> Very High<br>';
        return;
    }
    ...
    /* If there are no layers, clear legend */
    div.innerHTML = "";
}

/* Shows Intensity Legend by Default */
updateLegend();
...
fetch("data/tornado_paths.geojson")
    ...
    /* Catch function for troubleshooting (so many problems omg) */
    .catch(function (err) {
